...
GLog* GLog_new() {
    ...
    lObj->m_type = GString_new();
    lObj->m_side = GString_new();
    lObj->m_msg = GString_new();
...
static void GLog_delete(GLog* _this) {
    assert(_this);
    _this->clear(_this);
    _this->m_type->delete(_this->m_type);
    _this->m_side->delete(_this->m_side);
    _this->m_msg->delete(_this->m_msg);
...
static void GLog_setObj(GLog* _this, GLog* _obj) {
    assert(_this);
    _this->m_type->clone(_this->m_type, _obj->m_type);
    _this->m_side->clone(_this->m_side, _obj->m_side);
    _this->m_msg->clone(_this->m_msg, _obj->m_msg);
}
//===============================================
static GLog* GLog_loadToMap(GLog* _this, int i) {
    assert(_this);
...
static void GLog_showErrors(GLog* _this) {
    ...
    for(int i = 0; i < lMap->size(lMap); i++) {
        GLog* lObj = lMap->get(lMap, i);
        if(strcmp(lObj->m_type->m_data, "error")) continue;
        printf("[%-5s] : %s\n", lObj->m_type->m_data, lObj->m_msg->m_data);
    }
}
//===============================================
static void GLog_showLogs(GLog* _this) {
    ...
    for(int i = 0; i < lMap->size(lMap); i++) {
        GLog* lObj = lMap->get(lMap, i);
        if(strcmp(lObj->m_type->m_data, "log")) continue;
        printf("[%-5s] : %s\n", lObj->m_type->m_data, lObj->m_msg->m_data);
    }
}
//===============================================
static void GLog_showDatas(GLog* _this) {
    ...
    for(int i = 0; i < lMap->size(lMap); i++) {
        GLog* lObj = lMap->get(lMap, i);
        if(strcmp(lObj->m_type->m_data, "data")) continue;
        printf("[%-5s] : %s\n", lObj->m_type->m_data, lObj->m_msg->m_data);
    }
}
//===============================================
...
static int GLog_hasErrors(GLog* _this) {
    ...
    for(int i = 0; i < lMap->size(lMap); i++) {
        GLog* lObj = lMap->get(lMap, i);
        if(!strcmp(lObj->m_type->m_data, "error")) return 1;
    }
    return 0;
}
//===============================================
static int GLog_hasLogs(GLog* _this) {
    ...
    for(int i = 0; i < lMap->size(lMap); i++) {
        GLog* lObj = lMap->get(lMap, i);
        if(!strcmp(lObj->m_type->m_data, "log")) return 1;
    }
    return 0;
}
//===============================================
static int GLog_hasDatas(GLog* _this) {
    ...
    for(int i = 0; i < lMap->size(lMap); i++) {
        GLog* lObj = lMap->get(lMap, i);
        if(!strcmp(lObj->m_type->m_data, "data")) return 1;
    }
    return 0;
}
//===============================================
static GString* GLog_serialize(GLog* _this) {
    ...
    lDom->addData(lDom, lCode, "type", _this->m_type->m_data);
    lDom->addData(lDom, lCode, "side", _this->m_side->m_data);
    lDom->addData(lDom, lCode, "msg", _this->m_msg->m_data);
...
static void GLog_deserialize(GLog* _this, const char* _data) {
    ...
    _this->m_type->assign(_this->m_type, lDom->getData(lDom, lCode, "type"));
    _this->m_side->assign(_this->m_side, lDom->getData(lDom, lCode, "side"));
    _this->m_msg->assign(_this->m_msg, lDom->getData(lDom, lCode, "msg"));
