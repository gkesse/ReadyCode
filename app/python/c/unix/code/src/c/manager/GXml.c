...
static GString* GXml_getValue(GXml* _this);
...
static GString* GXml_getValue(GXml* _this) {
    ...
    lData->create(lData, (char*)xmlNodeGetContent(_this->m_node));
    return lData;
}
//===============================================
static GString* GXml_toString(GXml* _this) {
    ...
    GString* lData = GString_new();
    lData->create(lData, (char*)lBuffer);
    xmlFree(lBuffer);
    return lData;
}
//===============================================
...
static int GXml_loadXml(GXml* _this, const char* _data) {
    assert(_this);
    _this->clear(_this);
    _this->m_doc = xmlParseDoc(BAD_CAST(_data));
    if(!_this->m_doc) return 0;
    _this->m_node = xmlDocGetRootElement(_this->m_doc);
    if(!_this->m_node) return 0;
    return 1;
