...
GXml& GXml::loadXmlFile(const std::string& _filename) {
    ...
    if(!m_doc) {
        GERROR_ADD->addError(sformat("Erreur la methode (loadXmlFile) a echoue "
...
GXml& GXml::loadXml(const std::string& _data) {
    ...
    if(!m_doc) {
        GERROR_ADD->addError(sformat("Erreur la methode (loadXmlData) a echoue "
...
GXml& GXml::getRoot(const std::string& _nodename) {
    ...
    if(!m_node) {
        GERROR_ADD->addError(sformat("Erreur la methode (getRoot) a echoue "
...
std::string GXml::getNodeValue() const {
    if(!m_node) {
        GERROR_ADD->addError("Erreur la methode (getNodeValue) a echoue.");
        return "";
    }
